...
function App() {
  ...
  useEffect(() => {
    if (user) {
      debugLog('✅ User logged in:', user.email, 'Screen:', currentScreen);
    }
  }, [user, currentScreen]);
  ...
  const RecipeGenerationScreen = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        debugLog('🚀 Making OpenAI API call');
        debugLog('🔍 Request data:', requestData);
        debugLog('🔍 API URL:', `${API}/api/recipes/generate`);

        const response = await axios.post(`${API}/api/recipes/generate`, requestData);
        
        debugLog('🚀 Recipe generation response received');
        debugLog('✅ Recipe generated:', response.data?.title);
        debugLog('🔍 Recipe ID:', response.data?.id);
        debugLog('🔍 Shopping list items:', response.data?.shopping_list?.length);
        
        // Store recipe and navigate to detail
        window.currentRecipe = response.data;
        debugLog('🔍 Stored recipe and navigating to detail screen');
  ...
  const renderScreen = () => {
    ...
    switch (currentScreen) {
      ...
      case 'recipe-detail':
        debugLog('🚀 Rendering recipe detail screen');
        debugLog('🔍 Recipe:', window.currentRecipe?.title);
