...
class StarbucksAPITester:
    ...
    def validate_starbucks_response(self, data: dict, drink_type: str) -> tuple[bool, str]:
        """Validate Starbucks drink response format and content"""
        ...
        # Validate drink_name is creative/magical
        ...
        # Check for creative/whimsical words in name
        creative_words = ["frost", "swirl", "bloom", "cloud", "star", "moon", "magic", "dream", "velvet", "crystal", "mystic", "enchant", "glow", "shimmer", "sparkle", "galaxy", "cosmic", "celestial", "aurora", "rainbow"]
        name_lower = drink_name.lower()
        has_creative_element = any(word in name_lower for word in creative_words) or any(char.isupper() for char in drink_name[1:])
        
        # Validate description has vibe/mood
        description = data.get("description", "")
        if not description or len(description) < 10:
            return False, f"Description too short: '{description}'"
        
        # Validate ordering_script contains drive-thru format
        ordering_script = data.get("ordering_script", "")
        if not ordering_script or "hi" not in ordering_script.lower() or "can i get" not in ordering_script.lower():
            return False, f"Ordering script doesn't follow drive-thru format: '{ordering_script}'"
        
        # Validate modifications list
        modifications = data.get("modifications", [])
        if not isinstance(modifications, list) or len(modifications) < 1:
            return False, f"Modifications should be a non-empty list: {modifications}"
