...
curl https://recipe-cart-app-1.emergent.host/api/debug/verification-codes/prod.test@example.com
...
curl -X POST "https://recipe-cart-app-1.emergent.host/api/grocery/cart-options?recipe_id=RECIPE_ID&user_id=USER_ID" | jq .
```

### **7. Frontend Testing**
```bash
# Test frontend loads
curl -s https://recipe-cart-app-1.emergent.host/ | head -10

# Test service worker
curl -s https://recipe-cart-app-1.emergent.host/sw.js | head -5
```
...
curl -w "Total time: %{time_total}s\n" -o /dev/null -s https://recipe-cart-app-1.emergent.host/api/

# Recipe generation timing
time curl -X POST https://recipe-cart-app-1.emergent.host/api/recipes/generate \
  -H "Content-Type: application/json" \
...
curl https://recipe-cart-app-1.emergent.host/api/ | grep -i "password\|key\|secret"
```
