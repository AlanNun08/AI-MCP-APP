...
### Environment URLs
- **Preview Environment**: https://4a624c76-fc66-4a7c-91df-de079314ff82.preview.emergentagent.com
...
**Test Case**:
```bash
curl -X POST https://4a624c76-fc66-4a7c-91df-de079314ff82.preview.emergentagent.com/api/auth/verify-email \
  -H "Content-Type: application/json" \
  -d '{
    "email": "test@example.com",
...
**Test Case**:
```bash
curl -X POST https://4a624c76-fc66-4a7c-91df-de079314ff82.preview.emergentagent.com/api/starbucks/generate \
  -H "Content-Type: application/json" \
  -d '{
    "user_id": "test-user-id",
...
RECIPE_ID=$(curl -X POST https://4a624c76-fc66-4a7c-91df-de079314ff82.preview.emergentagent.com/api/recipes/generate \
  -H "Content-Type: application/json" \
  -d '{"user_id":"test","recipe_category":"snacks","cuisine_type":"cookies","servings":1,"difficulty":"easy"}' \
  | jq -r '.id')

# Then get cart options
curl -X POST "https://4a624c76-fc66-4a7c-91df-de079314ff82.preview.emergentagent.com/api/grocery/cart-options?recipe_id=$RECIPE_ID&user_id=test" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
**Test Case**:
```bash
curl -X POST https://4a624c76-fc66-4a7c-91df-de079314ff82.preview.emergentagent.com/api/grocery/custom-cart \
  -H "Content-Type: application/json" \
  -d '{
    "user_id": "test",
    "selected_products": [
...
```bash
#!/bin/bash
BASE_URL="https://4a624c76-fc66-4a7c-91df-de079314ff82.preview.emergentagent.com"
